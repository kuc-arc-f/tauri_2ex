...
import { itemsApi } from './Chat/api';
import ItemDialog from './Chat/ItemDialog';
import dbUtil from './Chat/db';
...
export default function App() {
    ...
    { 
      ...
      cell: ({ row }) => (
        <div className="rounded p-2 bg-white">
          <h3 className="text-1xl font-bold text-gray-400">User1:</h3>
          <span className="ms-2">{row.original.data.body}</span>
          <br />
  ...
  const handleEdit = async (item: Item) => {
    ...
    setEditingItem(item);
    setDialogOpen(true);
    const res = await itemsApi.threadList("chat_thread", item.id); 
    console.log(res);  
    setReplyItems(res)
  };
  ...
  const handleAddReply = async(todo) =>{
    try{
      const res = await itemsApi.threadList("chat_thread", Number(todo.id)); 
      console.log(res);  
      setReplyItems(res)
